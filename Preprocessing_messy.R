...
View(final_WS)
#loading the calprotectin database
All_CAL <- read.csv("Desktop/Project/All_CAL.csv", header = TRUE)
final_All <- left_join(All_CAL, final_WS, by = "ID")
View(final_All)
#filtering out missing/NA values which included any assigned 99 or 999 in this study
clean_WS <- final_All %>%
  filter(
...
head(Malaria)
#creating a dataframe with all the uninfected samples 
Uninfected <- read.csv("Desktop/Project/Malaria_negative.csv", header=TRUE)
Uninfected$ID <- as.numeric(Uninfected$ID)
head(Uninfected)
#joining the malaria positive data and the main dataset by the common column (ID) to create a dataframe with only the malaria positive data
...
#repeating this process with the uninfected controls
...
clean_Malaria_data$CAL..ug.g.feces. <- as.numeric(clean_Malaria_data$CAL..ug.g.feces.)
#applying the spearmans rank test to consider the relationship between infection status and bristol stool values 
...
#labelling each sample within the malaria positive file as "Positive"
malaria_pos_labels <- clean_Malaria_data  %>%
  select(ID) %>% 
  mutate(MALARIA = "Positive")

#labelling each sample within the uninfected file as "Negative"
malaria_neg_labels <- clean_Uninfected_data %>%
  select(ID) %>% 
  mutate(MALARIA = "Negative")  
#Combine them into a single key map
malaria_master_key <- bind_rows(malaria_pos_labels, malaria_neg_labels)

#This adds a 'MALARIA' column straight into the main database
...
plot_ready_data2 <- file_with_malaria %>%
  mutate(
    Malaria_Status = factor(MALARIA),
    Schisto_Status = factor(ifelse(SMA1A > 0 | SMA1B > 0, "Positive", "Negative"))
  ) %>%
  filter(!is.na(Malaria_Status) & !is.na(Schisto_Status))
TwoWay_model <- aov(log10(CAL..ug.g.feces.) ~ Malaria_Status * Schisto_Status, data = plot_ready_data2)
...
ggplot(plot_ready_data, aes(x = Infection_Group, y = CAL..ug.g.feces.)) +
  ...
  geom_signif(
    comparisons = list(c("Malaria+ / Schisto-", "Malaria+ / Schisto+")),
...
Ntokol_malaria_positive <- Ntokol_all_data %>% 
  filter(MALARIA == "Positive")
Ntokol_uninfected <- Ntokol_all_data %>% 
  filter(MALARIA == "Negative")

Ntokol_all_data$CAL..ug.g.feces. <- as.numeric(Ntokol_all_data$CAL..ug.g.feces.)
median(Ntokol_malaria_positive$CAL..ug.g.feces., na.rm = TRUE)
min(Ntokol_malaria_positive$CAL..ug.g.feces., na.rm = TRUE)
max(Ntokol_malaria_positive$CAL..ug.g.feces., na.rm = TRUE)
mean(Ntokol_malaria_positive$CAL..ug.g.feces., na.rm = TRUE)

median(Ntokol_uninfected$CAL..ug.g.feces., na.rm = TRUE)
min(Ntokol_uninfected$CAL..ug.g.feces., na.rm = TRUE)
max(Ntokol_uninfected$CAL..ug.g.feces., na.rm = TRUE)
mean(Ntokol_uninfected$CAL..ug.g.feces., na.rm = TRUE)
